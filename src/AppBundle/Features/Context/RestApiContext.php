...
class RestApiContext implements Context
{
     ...
     *
     * @When /^(?:Yo )?envio una solicitud "([A-Z]+)" a "([^"]+)"$/
     */
    public function yoEnvioUnaSolicitud($method, $url)
    {
     ...
     *
     * @Then el codigo de respuesta debe de ser :arg1
     */
     ...
     *
     * @Then /^(?:la )?respuesta debe contener un json:$/
     */
    public function laRespuestaDebeContenerUnJson(PyStringNode $jsonString)
    {
        $etalon = json_decode($this->replacePlaceHolder($jsonString->getRaw()), true);
        $actual = json_decode($this->response->getBody()->getContents(),true);
    ...
    /**
     * @Then la cabecera de respuesta :header debe ser igual a :value
     */
    public function laCabeceradeRespuestaDebeSerIgualA($header, $value)
    {
